...
if($action=="getRecipeOfDay"){
	getRecipeOfDay($Host, $User, $DBPassword, $DBName, $table_1, $table_2, $table_3);
}

function getRecipeOfDay($Host, $User, $DBPassword, $DBName, $table_1, $table_2, $table_3){

	$Link = mysqli_connect($Host, $User, $DBPassword, $DBName);
	$Query = "SELECT * FROM $table_2 ORDER BY RAND() LIMIT 1";
	...
	if($Result = mysqli_query($Link, $Query)){
		...
		while($row = mysqli_fetch_array($Result)){
			...
			$Query2 = "SELECT * FROM $table_3 WHERE recipe_id ='".$recipe_id."' ";
